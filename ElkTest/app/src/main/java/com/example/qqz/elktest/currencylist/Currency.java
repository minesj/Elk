...
import com.example.qqz.elktest.BaseApplication;
import com.example.qqz.elktest.gson.CalculatorUtil;
import com.example.qqz.elktest.helper.AppHelper;
...
public class Currency implements Serializable {
    ...
    public static final String SECTION_FAV = "常用";
   ...
   /* public void setInputResult(String inputResult) {
    ...
    public void calculateResult(int decimal) {
        if (inputResult == null || inputResult.isEmpty()) {
            ...
            if (defaultValue.equals("1")) {
                result = priceToString(1 * rate, decimal);
            } else if (defaultValue.equals("10")) {
                result = priceToString(10 * rate, decimal);
            } else {
                result = priceToString(100 * rate, decimal);
            }
        } else if (inputResult.length() > 0) {
            if (CalculatorUtil.hasOperator(inputResult)) {
                Double cal = CalculatorUtil.calc(inputResult).doubleValue();
                result = priceToString(cal * rate, decimal);
            } else {
                try {
                    result = priceToString(
    ...
    public String getShowResult(int decimal) {
        calculateResult(isFiatCurrency() ?
                getChooseCurrencyDecimal(BaseApplication.getApplication(), ShareDataHelper.KEY_DEFAULT_FIAT_VALUE) :
                getChooseCurrencyDecimal(BaseApplication.getApplication(), ShareDataHelper.KEY_DEFAULT_CRYPTO_VALUE));
        return result;
    }
    ...
    public static String priceToString(Double price, int decimal) {
        String pattern = "#,###";
        if (decimal == 1) {
            pattern = "#,###.0";
        } else if (decimal == 2) {
        ...
        DecimalFormat nf = (DecimalFormat) NumberFormat.getInstance(new Locale("zh", "CN"));
        nf.applyPattern(pattern);
        String strPrice = nf.format(price);
        if (!strPrice.contains(",") && strPrice.length() > 0
                && Math.abs(Double.parseDouble(strPrice)) < 1) {
            strPrice = "0" + strPrice;
            strPrice = strPrice.replace("0-", "-0");
        }
        return strPrice;
    }
    public static int getChooseCurrencyDecimal(Context context, String type){
        return ShareDataHelper.getIntValueWithDefault(context, type,
                TextUtils.equals(ShareDataHelper.KEY_DEFAULT_FIAT_VALUE, type) ? 2 : 8);
    }
}
